...
describe('SimplifierClient', () => {
  ...
  describe('getConnectorCall', () => {
    ...
    it('should preserve other category values', async () => {
      ...
      const result = await client.getConnectorCall('TestConnector', 'mixedCall', "test");

      expect(result.connectorCallParameters).toHaveLength(4);
      expect(result.connectorCallParameters[0].dataType.category).toBe('base');
      expect(result.connectorCallParameters[1].dataType.category).toBe('struct');
      expect(result.connectorCallParameters[2].dataType.category).toBe('domain');
      expect(result.connectorCallParameters[3].dataType.category).toBe('collection');
    });
  });
